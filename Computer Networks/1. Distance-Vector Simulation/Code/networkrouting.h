...
// Modified by Hu Yang on 12/11/21
//
...
#ifndef DV_ROUTING_NETWORKROUTING_H
#define DV_ROUTING_NETWORKROUTING_H
...
public:
    int16_t thres = 0; // The threshold should be the sum of all edges in the topology.
    ...
    /**
     * neigh_count and neigh_connected go hand-in-hand so there might be a bit redundancy. However, it is faster and makes more sense
     * (and thus the code more readable) to find out if a neighbor is connected by checking neigh_connected than by checking its
     * ping counter. So, neigh_connected is preserved and maintained as this new feature joins.
     */
    std::map<std::string, std::int16_t> neigh_count;
    ...
    /* This method returns a beautiful "hh:mm:ss" string of current time. */
