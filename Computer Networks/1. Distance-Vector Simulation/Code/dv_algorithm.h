...
class DV_Routing {
    int thres;
public:
    bool inf_timer_enabled = false;
    clock_t inf_timer_start;
    DV_Routing(int interval, int thres);
    ~DV_Routing();
    ...
    int send_rping_msg(std::string to_node_id, TransportLayer *p_t_l, Network_Info *p_net_info);

    int decode_dv_msg(std::string received_msg, DV_Msg *p_dv_msg);

    int increase_counter(Network_Info * p_net_info);
    static int reset_counter(Network_Info * p_net_info, std::string node_id);
